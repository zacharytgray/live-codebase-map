...
- **Store is worktree-local and git-excluded in v1 (2026-07-18).** `.codemap/` goes in `.git/info/exclude` (not the observed repo's `.gitignore` — leave the observed repo untouched). Every event carries `branch` + `commit`; the view reconciles cross-branch history via provenance rather than git merging the log. The "JSONL git-merges well" argument from the research is deferred to multi-machine/team scenarios. **Now an open question (2026-07-18):** tracked vs local is a live discussion point for Zach + Dallas — full pros/cons in `docs/llm-dev-integration.md` §open-questions (tracked = per-branch accuracy + shared annotations via union merge driver; local = no diff noise, no agent narration in public history). `state.json` and `telemetry.jsonl` stay local under any outcome. JSONL stays regardless: append-only, human-readable, trivially parseable.
...
- **LLM summaries, opt-in per repo (2026-07-18, Zach's explicit request).** `codemap summarize` makes Layer-3 consolidation concrete: per-file 1-2 sentence purpose summaries from deterministic context (entities, edges, existing annotations) + file text, backend `claude -p` (haiku default, overridable). Refresh trigger: no summary yet, OR ≥3 modified events since the last one, OR cumulative |delta_loc| ≥ 30% of file loc. Stored as `annotation` origin `llm-summary`, `source: "consolidation"`, `model` recorded. Never runs from capture hooks — invariant 1 is about the critical path, and this stays off it.
